...
function getQuestionsArray(json, prefix) {
  var askFields = [];
  prefix = prefix || '';
  ...
  Object.keys(json).forEach(function(field) {
    if (json[field] === '<askme>') {
      askFields.push({
        name: field,
        description: (prefix + '/' + field).cyan
      });
    } else if (typeof json[field] === 'object') {
      var childQuestions = getQuestionsArray(json[field], field);
      askFields = askFields.concat(childQuestions);
    }
  });
